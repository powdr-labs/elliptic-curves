...
impl FieldElement10x26 {
    ...
    pub(crate) fn from_bytes_unchecked_le(bytes: &[u8; 32]) -> Self {
        ...
        let w0 = u32::from_le_bytes(bytes[0..4].try_into().unwrap()) & 0x03ffffff;
        ...
        let w2 = (((bytes[6] >> 4) as u32) & 0xf)
            ...
            | ((bytes[8] as u32) << 12)
            | (((bytes[9] & 0x3f) as u32) << 20);
        let w3 = (u32::from_le_bytes(bytes[9..13].try_into().unwrap()) >> 6) & 0x03ffffff;
        // let w3 = (((bytes[9] >> 6) as u32) & 0x3)
        //     | ((bytes[10] as u32) << 2)
        //     | ((bytes[11] as u32) << 10)
        //     | ((bytes[12] as u32) << 18);
        let w4 = u32::from_le_bytes(bytes[13..17].try_into().unwrap()) & 0x03ffffff;
        ...
        let w6 = (((bytes[19] >> 4) as u32) & 0xf)
            ...
            | ((bytes[21] as u32) << 12)
            | (((bytes[22] & 0x3f) as u32) << 20);
        let w7 = (u32::from_le_bytes(bytes[22..26].try_into().unwrap()) >> 6) & 0x03ffffff;
        // let w7 = (((bytes[22] >> 6) as u32) & 0x3)
        //     | ((bytes[23] as u32) << 2)
        //     | ((bytes[24] as u32) << 10)
        //     | ((bytes[25] as u32) << 18);
        let w8 = u32::from_le_bytes(bytes[26..30].try_into().unwrap()) & 0x03ffffff;
